...
class Solution:
    ...
    def find_scc(self, res: List[int], adj_list: List[List[int]], scc_ids: List[int], visited: List[bool], cur_rank: int, prev_node: int, cur_node: int) -> None:
        """
        ...
        for next_node in adj_list[cur_node]:
            ...
            if not visited[next_node]:
                self.find_scc(res, adj_list, scc_ids, visited, cur_rank + 1, cur_node, next_node)
    ...
    def find_adj_list(self, n: int, connections: List[List[int]]) -> List[List[int]]:
        """
